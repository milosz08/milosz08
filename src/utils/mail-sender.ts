 ...
 * Last modified: 23/04/2023, 00:19
...
class MailSender {
    ...
    async sendEmail(sendTo: string, subject: string, template: string, replacements: EmailReplacements): Promise<void> {
        const transporter = nodemailer.createTransport({
            host: config.SMTP_HOST,
            port: Number(config.SMTP_PORT),
            secure: typeof config.SMTP_SSL === 'boolean' ? config.SMTP_SSL : /^true$/i.test(config.SMTP_SSL),
            auth: {
    ...
    private generateTemplate(templateName: string, replacements: EmailReplacements): string {
        const source = fs
            .readFileSync(utilities.getProjectRootPath(this.TEMPLATES_PATH + "/" + templateName + ".html"))
            .toString();
