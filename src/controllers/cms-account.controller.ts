 ...
 *   Last updated at: 2023-08-31, 19:48:11
...
class CmsAccountController {
  async getAccountsPage(req: Request, res: Response): Promise<void> {
    ...
    const where = {
      $or: [{ login: regex }, { email: regex }, { role: regex }],
    };
  ...
  async postCreateAccountPage(req: Request, res: Response): Promise<void> {
    ...
    try {
      ...
      await MailSender.sendEmail(
        email,
        `Welcome in CMS system! (${login})`,
        Constant.AFTER_CREATED_ACCOUNT_MAIL_TEMPLATE,
        replacements
      );
      ...
      logger.info(
        `Successfull created user account and send email message. Account: ${JSON.stringify(
          savedUser
        )}`
      );
  ...
  async postUpdateAccountPage(req: Request, res: Response): Promise<void> {
    ...
    try {
      ...
      const restOfAdmins = await UserModel.find({
        $and: [{ role: Constant.ADMIN }, { login: { $ne: user.login } }],
      });
      if (restOfAdmins.length === 0 && role === Constant.MODERATOR) {
        throw new Error(
          'In CMS system must be at least one user with administrator permissions.'
        );
      }
      user.login = login;
      user.email = email;
      ...
      logger.info(
        `Successfull updated user account. Account: ${JSON.stringify(
          updatedUser
        )}`
      );
      res.redirect('/cms/accounts');
      ...
      logger.error(
        `Failure updated account informations. Cause: ${ex.message}`
      );
      res.render(path, {
        title,
        layout,
  ...
  async getDeleteAccountRedirect(req: Request, res: Response): Promise<void> {
    ...
    try {
      ...
      if (restOfAdmins.length === 0) {
        throw new Error(
          'You cannot delete last user with administrator permissions.'
        );
      }
      await UserModel.findByIdAndDelete(user._id);
      alertMessage = `User account with login <strong>${user.login}</strong> was successfully deleted.`;
      logger.info(
        `Successfull deleted user account. Account: ${JSON.stringify(user)}`
      );
    } catch (ex: any) {
  ...
  async getRevokeAccountPasswordRedirect(
    req: Request,
    res: Response
  ): Promise<void> {
    const { accountId } = req.params;
    ...
    try {
      ...
      await MailSender.sendEmail(
        user.email,
        `Your password was revoked! (${user.login})`,
        Constant.AFTER_REVOKED_PASSWORD_MAIL_TEMPLATE,
        replacements
      );
      ...
      logger.info(
        `Successfull revoked user password for ${user.login} and send email message`
      );
    } catch (ex: any) {
      alertType = Constant.ALERT_DANGER;
      alertMessage = ex.message;
