 ...
 *   Last updated at: 2023-08-31, 19:48:07
...
class AuthController {
  ...
  async postLoginPage(req: Request, res: Response): Promise<void> {
    ...
    try {
      ...
      if (!user) {
        throw new Error(
          'Invalid email and/or password. Try again with another credentials.'
        );
      }
  ...
  getLogoutRedirect(req: Request, res: Response): void {
    ...
    logger.info(
      `Successfully logout from: '${req.session.loggedUser?.login}' account`
    );
  ...
  async postFirstLoginPage(req: Request, res: Response): Promise<void> {
    ...
    try {
      const user = await UserModel.findOne({
        login: req.session.loggedUser?.login,
      });
      ...
      logger.info(
        `Successfully change default password for: '${user.login}' account`
      );
      res.redirect('/cms/projects');
    } catch (ex: any) {
  ...
  getRequestChangePasswordPage(req: Request, res: Response): void {
    ...
    res.render(path, {
      ...
      pageAlert: utilities.extractAlertAndDestroy(
        req,
        AlertTypeId.REQUEST_CHANGE_PASSWORD_PAGE
      ),
    });
  }

  async postRequestChangePasswordPage(
    req: Request,
    res: Response
  ): Promise<void> {
    const { path, title, layout } = Constant.AUTH_REQUEST_CHANGE_PASSWORD_EJS;
    ...
    try {
      ...
      const replacements: EmailReplacements = {
        ...
        linkWithToken: `${Utilities.getFullUrl(
          req
        )}/change-password/${generatedToken}`,
        regenerateToken: `${Utilities.getFullUrl(req)}/request-change-password`,
      };
      await MailSender.sendEmail(
        user.email,
        `Request for change password (${user.login})`,
        Constant.CHANGE_PASSWORD_MAIL_TEMPLATE,
        replacements
      );
      ...
      logger.info(
        `Successfully send request for change password for: '${user.login}' account`
      );
  ...
  async getChangePasswordPage(req: Request, res: Response): Promise<void> {
    ...
    const isExpired =
      otaToken === null ||
      otaToken.isExpired ||
      otaToken.expiredAt < new Date();
    res.render(path, { title, layout, tokenIsValid: !isExpired, token });
  }
  ...
  async postChangePasswordPage(req: Request, res: Response): Promise<void> {
    ...
    try {
      ...
      if (isInvalid) {
        throw new Error(
          'Token is invalid, expired or already used. Try with another token'
        );
      }
