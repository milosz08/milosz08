 ...
 * Last modified: 26/04/2023, 16:32
...
class CmsSocialLinksController {
    ...
    async getSocialLinksPage(req: Request, res: Response): Promise<void> {
        ...
        const paginationUrl = q ? `/cms/social-links?q=${q}&` : "/cms/social-links?";

        const regex = { $regex: q || "", $options: "i" };
        const where = { $or: [ { paraphrase: regex }, { link: regex }, { iconClass: regex } ] };
        ...
        res.render(path, { title, layout,
            pageAlert: utilities.extractAlertAndDestroy(req, AlertTypeId.CMS_SOCIAL_LINKS_PAGE),
            socialLinks,
            page: selectedPage,
            pagesCount,
            resultsCount,
    ...
    async getDeleteSocialLinksRedirect(req: Request, res: Response): Promise<void> {
        const { socialLinkId } = req.params;
